    ...
    var SBChat = {
        ...
        showArticles_: function (items, panel, payload) {
            ...
            for (var i = 0; i < items.length; i++) {
                ...
                code += `<div data-id="${items[i].id}"${is_article ? '' : ' data-is-category="true"'}><div>${is_article ? items[i].title : (language && items[i].languages && items[i].languages[language] ? items[i].languages[language].title : items[i].title)}</div><span>${is_article ? items[i].content : (language && items[i].languages && items[i].languages[language] ? items[i].languages[language].description : items[i].description)}</span></div>`;
            }
    ...
    $(document).ready(function () {
        ...
        if (typeof SB_INIT_URL != ND) {
            if (SB_INIT_URL.indexOf('.js') < 0) {
                const SB_INIT_URL += '/js/main.js?v=' + version;
            }
            url_full = SB_INIT_URL;
            console.log('SB_INIT_URL', SB_INIT_URL);
        } else {
